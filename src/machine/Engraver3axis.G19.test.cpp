...
class Engraver3axisG19Tests : public TestSuite {
Q_OBJECT
private slots:
    ...
    void testSwitchToYZ() {
        ...
        auto block = parser.blocks()[0];
        auto result = engraver.parseG19(block);
        auto operations = engraver.getOperations();
        auto errors = engraver.getErrors();
        QCOMPARE(result, true);
        QCOMPARE(operations.size(), 0);
        QCOMPARE(errors.size(), 0);
        QCOMPARE(engraver.getState().planeSelection, State::YZ);
        QCOMPARE(engraver.getState().lastGCodeCommand, 19);
        QCOMPARE(engraver.getState().lastGCodeSubcommand, 0);
    }
};

static Engraver3axisG19Tests T_Engraver3axisG19Tests;

#include "Engraver3axis.G19.test.moc"
